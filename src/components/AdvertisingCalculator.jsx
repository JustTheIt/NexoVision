...
const AdvertisingCalculator = () => {
  const fixedDailyBudget = 180; // Fixed daily budget
  const [duration, setDuration] = useState(7); // Start with 1 day

  // Constants for calculations
  const estimatedDailyReach = Math.round(fixedDailyBudget * 15); 
  const totalCost = fixedDailyBudget * (duration === '' ? 1 : duration);
  const totalEstimatedReach = estimatedDailyReach * (duration === '' ? 1 : duration);
  ...
  const handleSubmit = () => {
    const message = `Advertising Campaign Inquiry\n\n` +
                   `Budget Details:\n` +
                   `Daily: ${formatCurrency(fixedDailyBudget)}\n` +
  ...
  const handleCustomStrategy = () => {
    const message = `Custom Advertising Strategy Request\n\n` +
                   `Current Plan:\n` +
                   `Daily Budget: ${formatCurrency(fixedDailyBudget)}\n` +
                   `Duration: ${duration === '' ? 1 : duration} days\n\n` +
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Allow empty input (blank)
                    if (value === '') {
                      setDuration('');
                    } 
                    // Otherwise parse as number and ensure it's at least 1
                    else {
                      const numValue = Number(value);
                      setDuration(isNaN(numValue) ? 1 : Math.max(1, numValue));
                    }
                  }}
                  onBlur={(e) => {
                    // If empty on blur, default to 1 day
                    if (e.target.value === '') {
                      setDuration(7);
                    }
                  }}
                ...
                {duration !== '' && duration < 7 && (
                  <p className="mt-2 text-sm text-red-600 italic">
                    For optimal results, we recommend running your campaign for at least 7 days.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatCurrency(fixedDailyBudget)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Campaign Duration:</span>
                  <span className="font-medium">{duration === '' ? 1 : duration} day{duration !== 1 && duration !== '' ? 's' : ''}</span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            <button 
              className="w-full md:w-auto px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition duration-300"
              onClick={handleSubmit}
            >
              Submit Inquiry
            </button>
            <button 
              className="w-full md:w-auto px-6 py-2 bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 rounded-lg transition duration-300"
